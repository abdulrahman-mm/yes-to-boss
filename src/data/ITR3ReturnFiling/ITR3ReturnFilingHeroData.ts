import { serviceHeroTypes } from "@/types";

export const ITR3ReturnFilingHeroData: serviceHeroTypes = {
  heading: "ITR-3 Return Filing",
  subHeading:
    "We simplify ITR-3 return filing for individuals and HUFs earning from business, profession, capital gains, or foreign assets, ensuring expert-guided, accurate, and timely compliance.",
  question: "What Is ITR-3 Return Filing?",
  answer:
    "ITR-3 is an income tax return form meant for individuals and Hindu Undivided Families (HUFs) who earn income from a proprietary business, profession, or other complex sources such as capital gains, multiple properties, foreign assets, or as directors in a company.",
    image: "/images/x.svg",

};
